...
const SearchResultItem = ({ result, onSelect }: SearchResultItemProps) => {
  ...
  return (
    <CommandItem
      value={result.name}
      onSelect={onSelect}
      className="flex items-center py-4 px-3 cursor-pointer rounded-lg hover:bg-neutral-800/50 transition-colors duration-200"
    >
      {icon}
      <div className="flex flex-col space-y-1">
        <span className="text-base font-medium text-white">{result.name}</span>
